...
function Player:init(map)
    ...
    self.sounds = {
        ...
        ['levelComplete'] = love.audio.newSource('music/levelComplete.wav', 'static')
    }
    ...
    self.behaviors = {
        ['idle'] = function(dt)
            ...
            if love.keyboard.wasPressed('space') or love.keyboard.wasPressed('w') or love.keyboard.wasPressed('up') then
...
function Player:update(dt)
    ...
    self.currentFrame = self.animation:getCurrentFrame()
    self.x = math.max(0, math.min(self.x + self.dx * dt, map.mapWidthPixels - self.width))
    ...
    -- restart if dead
    if self.y > map.mapHeightPixels then
        love.load()
    end
end
...
function Player:calculateJumps()
    ...
    if self.dy < 0 then
        ...
        if self:blocksAboveAreSolid() then
            -- reset y velocity
            self.dy = 0
...
function Player:blockAboveIsSolid(x)
    for _, val in pairs(NOT_SOLID) do -- Table iteration.
        if self.map:tileAt(x, self.y).id == val then
            return false
        end
    end
    return true
...
function Player:blocksAboveAreSolid()
    if self:blockAboveIsSolid(self.x) or self:blockAboveIsSolid(self.x + self.width - 1) then
        return true
    end
    return false
end
...
function Player:checkFlagCollision()
    if not self.flagHasBeenTouched then
        local flagParts = {FLAG_BASE, FLAG_POLE, FLAG_TOP}

        -- cornes of the sprite, topright, topleft, bottomright, bottomleft
        local tileAboveRight = self.map:tileAt(self.x + self.width, self.y).id
        local tileAboveLeft = self.map:tileAt(self.x, self.y).id
        local tileBelowRight = self.map:tileAt(self.x, self.y + self.height).id
        local tileBelowLeft = self.map:tileAt(self.x + self.width, self.y + self.height).id

        -- iterate and return true if our tile type matches
        for _, v in ipairs(flagParts) do
            if tileAboveRight == v or tileAboveLeft == v or tileBelowRight == v or tileBelowLeft == v then
                self.flagHasBeenTouched = true
